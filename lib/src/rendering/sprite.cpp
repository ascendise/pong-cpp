...
Sprite::Sprite(std::unique_ptr<ITexture> &&texture, int spriteCount, float duration)
    : spriteCount(spriteCount), texture(std::move(texture)), avgFrameTime(duration / (float)spriteCount),
...
const SDL_Rect
Sprite::getNextRect(std::chrono::time_point<std::chrono::high_resolution_clock, std::chrono::nanoseconds> time) {
...
bool Sprite::isPastFrameTime(
    std::chrono::time_point<std::chrono::high_resolution_clock, std::chrono::nanoseconds> time) const {
...
std::unique_ptr<ITexture> &Sprite::getTexture() { return this->texture; }
} // namespace pong::rendering