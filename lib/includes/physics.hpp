...
namespace pong::physics {
	...
	public:
	  ...
	  RigidBody(math::Vector2D velocity, float bounce)
	...
	public:
	  BoxCollider(world::Position position, math::Vector2D area)
	      : position(std::move(position)), area(area) {}
	...
	class CollisionEventProcessor : public world::events::EventProcessor {
	  math::Vector2D
	  calculateVelocityAfterCollision(float angle, const RigidBody &targetRigidBody,
	                                  float bounce);
	...
	struct Collision {
	  ...
	  Collision(world::Entity &entity1, world::Entity &entity2)
	...
	public:
	  CollisionSystem(world::events::IEventQueuePort &eventQueue)
