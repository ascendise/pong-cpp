...
void World::registerEntity(std::vector<std::shared_ptr<Component>> components) {
...
void World::registerSystem(std::unique_ptr<System> &&system) { this->systems.push_back(std::move(system)); }
...
IReadOnlyClock &World::getClock() const { return *this->clock; }

events::IEventQueuePort &World::getEventQueue() { return *this->eventQueue; }
} // namespace pong::world