...
void Prefabs::registerBackground(world::World &world, rendering::SDLRenderer &renderer) {
  ...
  rendering::Sprite sprite(std::make_unique<rendering::Texture>(std::move(texture)), 1, 0);
...
void Prefabs::registerBall(world::World &world, rendering::SDLRenderer &renderer) {
  std::vector<std::shared_ptr<world::Component>> components;
  auto position = std::make_shared<world::Position>(0, 0);
  components.push_back(position);
  rendering::Texture texture = rendering::Texture::loadTexture(*renderer, "../../assets/anim_test.png");
  rendering::Sprite sprite(std::make_unique<rendering::Texture>(std::move(texture)), 4, 0.1);
  components.push_back(std::make_shared<rendering::Sprite>(std::move(sprite)));
  auto rigidBody = std::make_shared<physics::RigidBody>(math::Vector2D(200, 100), 0);
  components.push_back(rigidBody);
  auto collider = std::make_shared<physics::BoxCollider>(*position, math::Vector2D(64, 64));
...
void Prefabs::registerPaddle(world::World &world, rendering::SDLRenderer &renderer) {
  std::vector<std::shared_ptr<world::Component>> components;
  auto position = std::make_shared<world::Position>(-500, 0);
  components.push_back(position);
  auto rigidBody = std::make_shared<physics::RigidBody>(math::Vector2D(0, 0), 1);
  components.push_back(rigidBody);
  components.push_back(std::make_shared<game::controls::PaddlePlayerControl>(*rigidBody));
  auto collider = std::make_shared<physics::BoxCollider>(*position, math::Vector2D(50, 400));
  components.push_back(collider);
  auto texture = rendering::Texture::loadTexture(*renderer, "../../assets/wall.png");
  auto sprite = std::make_shared<rendering::Sprite>(std::make_unique<rendering::Texture>(std::move(texture)), 1, 0);
...
void Prefabs::registerInvisibleWall(world::World &world, pong::rendering::SDLRenderer &renderer,
  ...
  wallComponents.push_back(std::make_shared<physics::RigidBody>(math::Vector2D(0, 0), 1));
  ...
  wallComponents.push_back(std::make_shared<rendering::Sprite>(std::move(texture), 1, 0));
