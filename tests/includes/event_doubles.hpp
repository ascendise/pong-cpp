...
public:
  void enqueue(std::shared_ptr<Event> &&event) override { this->events.push_back(std::move(event)); }

  std::vector<std::shared_ptr<Event>> const &getEvents() const { return this->events; }
};
} // namespace pong::world::events::testing
#endif
